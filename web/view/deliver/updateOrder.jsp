        ...
        <main class="app-content">
            <div class="app-title">
                <ul class="app-breadcrumb breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="#">Cập nhật đơn hàng</a></li>
                </ul>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="tile">
                        ...
                        <div class="tile-body">
                            <form class="row" name="orderForm" action="updateorder" method="POST">
                                <input type="hidden" value="add" name="action"/>
                                <div class="row form-group col-md-12">
                                    <div class="form-group col-md-1">
                                        <label class="control-label">Mã số</label>
                                        <input class="form-control" type="text" name="orderId" value="${order.id}" readonly>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="exampleSelect1" class="control-label">Địa chỉ giao hàng</label>
                                        <input class="form-control" type="text" name="orderAddress" value="${order.deliveryAddress}" readonly>
                                    </div>
                                    <div class="form-group col-md-2">
                                        <label for="exampleSelect1" class="control-label">Số điện thoại giao hàng</label>
                                        <input class="form-control" type="text" name="orderPhone" value="${order.deliveryPhone}" readonly>
                                    </div>
                                </div>

                                <div class="row form-group col-md-12">
                                    <div class="form-group col-md-2">
                                        <label for="exampleSelect1" class="control-label">Tên người nhận</label>
                                        <input class="form-control" type="text" name="orderName" value="${order.recipientName}" readonly>
                                    </div>
                                    <div class="form-group col-md-2">
                                        <label for="exampleSelect1" class="control-label">Phương thức thanh toán</label>
                                        <input class="form-control" type="text" name="orderPayment" value="${order.payment}" readonly>
                                    </div>
                                    <div class="form-group col-md-2">
                                        <label for="exampleSelect1" class="control-label">Trạng thái</label>
                                        <select class="form-control" name="orderStatus">
                                            <c:forEach items="${listStatus}" var="status">
                                                <option value="${status.id}" <c:if test="${status.id eq order.status.id}">selected</c:if>>${status.name}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </div>
